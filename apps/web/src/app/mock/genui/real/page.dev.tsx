...
import { ToolChips } from '@magic-resume/genui';
...
function InterruptProbe() {
  ...
  return (
    <div>
      ...
      />
      <pre className="mt-3 rounded-lg bg-sunk p-3 font-mono text-mr-label leading-relaxed text-secondary">
...
function ChoiceRoutingProbe() {
  ...
  const render = (label: string, args: Record<string, unknown>) => {
    ...
    return (
      <div className="flex-1">
        <div className="mb-2 font-mono text-mr-label text-muted">
          {label} → {kind}
          {/* i18n-ignore：仅开发验收页 */}
        </div>
        {Card && (
  ...
  return (
    <div>
      ...
      <pre className="mt-3 rounded-lg bg-sunk p-3 font-mono text-mr-label text-secondary">
 ...
 *
 * 走的是生产组件本身，不是 GenUI 那个自演示的 StreamingText——要验的正是
  ...
  return (
    <div>
      <button
        ...
        className="mb-3 rounded-lg border border-hairline px-2.5 py-1 text-mr-overline text-secondary"
      >
  ...
  return (
    <div className="dark min-h-screen bg-desk px-6 py-10 text-primary">
      <div className="mx-auto max-w-3xl">
        ...
        <p className="mt-1 text-mr-caption text-secondary">
          数据来自 Core dist 的{/* i18n-ignore */} <code className="font-mono text-mr-overline">event.mapper{/* i18n-ignore */}</code> 实际产出，
          非手写样例。下方是生产组件本身。
          {/* i18n-ignore：仅开发环境 */}
        </p>

        <div className="mt-6 rounded-xl border border-hairline p-4">
          <div className="mb-3 font-mono text-mr-label uppercase tracking-wider text-muted">
            后端原始事件{/* i18n-ignore：仅开发验收页 */}
          </div>
          <pre className="max-h-40 overflow-auto rounded-lg bg-sunk p-3 font-mono text-mr-label leading-relaxed text-secondary">
        ...
        <div className="mt-6 rounded-xl border border-hairline p-4">
          ...
          <p className="mb-3 text-mr-overline text-muted">
