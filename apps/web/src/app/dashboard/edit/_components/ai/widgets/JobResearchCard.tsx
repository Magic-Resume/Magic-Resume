...
type Group = { key: string; items: string[]; accent: string; actionable?: boolean };
 ...
 */
export default function JobResearchCard({ instance, onAction }: WidgetProps) {
        ...
        {groups.map((g) => (
          <WidgetItem key={g.key}>
            <div className="mb-1.5 flex items-center gap-2">
              <span className="h-3 w-1 rounded-full" style={{ background: g.accent }} />
              <span className="text-[11px] font-medium text-secondary">
                {t(`aiLab.widgets.research.groups.${g.key}`, { defaultValue: g.key })}
              </span>
            </div>
            <ul className="space-y-1 pl-3">
              {g.items.map((item) =>
                g.actionable ? (
                  <li key={item}>
                    <button
                      ...
                      onClick={() =>
                        onAction({
                          type: 'submit',
                          values: { follow_up: t('aiLab.widgets.research.followUp', { item }) },
                        })
                      }
                  ...
                  <li key={item} className="text-xs leading-relaxed text-secondary">
                    {item}
                  </li>
                ),
              )}
