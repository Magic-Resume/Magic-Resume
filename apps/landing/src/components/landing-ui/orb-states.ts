import { UNIFORM_SEED, U } from './liquid-orb-shader';

/** 两路电平 0-1。落地页没有麦克风,由调用方给脚本化包络。 */
...
export type OrbPhase =
  'idle' | 'connecting' | 'listening' | 'thinking' | 'speaking' | 'finished';
  ...
  switch (phase) {
    ...
    case 'idle': {
      ...
      return still({
        ...
        speedScale: 0.3,
        radius:
          B.radius * (1 - IDLE_BREATH_DEPTH + 2 * IDLE_BREATH_DEPTH * breath),
