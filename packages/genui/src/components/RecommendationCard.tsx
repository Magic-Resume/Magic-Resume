...
const SIGNAL: Record<RecommendationConfidence, { bars: number; tone: string }> = {
  high: { bars: 3, tone: "var(--mr-success)" },
  medium: { bars: 2, tone: "var(--mr-warning)" },
  low: { bars: 1, tone: "var(--mr-muted)" },
};
    ...
    <span className="flex items-end gap-0.5">
      {[0, 1, 2].map((bar) => (
        <span
          ...
          style={{ height: 10, background: bar < signal ? tone : "var(--mr-line-strong)" }}
        />
      ))}
    </span>
  );
  ...
  return (
    <div className="w-full max-w-95 overflow-hidden rounded-mr-card bg-mr-surface shadow-mr-panel">
      <div className="primitive-card-pad">
        <span className="text-mr-caption font-semibold text-mr-ink">{message}</span>
        <p
          key={active.label}
          className="mt-1.5 min-h-12 text-mr-caption leading-relaxed text-mr-ink-secondary"
          style={{ animation: "fade-in 180ms ease-out both" }}
        >
          <span className="font-medium text-mr-ink">{active.label}</span>
          {active.why ? <span className="block mt-0.5">{active.why}</span> : null}
        </p>
      </div>

      {/* alternatives drawer — a distinctly new section of the card
          底色用**叠加**（bg-mr-surface-soft）而不是原版的 `bg-mr-sunk`：`inset` 在我们的深色主题里是
      ...
      {others.length > 0 && (
        ...
        >
          <div className="overflow-hidden">
            <div className="border-t border-mr-line bg-mr-surface-soft px-2 py-2">
              <p className="px-1.5 pb-1 text-mr-label font-medium text-mr-muted">{labels.others}</p>
              {others.map(({ o, i }) => {
                const s = o.confidence ? SIGNAL[o.confidence] : { bars: 0, tone: "var(--mr-muted)" };
                return (
                  <button
                    key={o.label}
                    type="button"
                    ...
                    className="flex w-full items-center gap-2.5 rounded-mr-control px-1.5 py-1.5
                      text-left transition-colors duration-100 enabled:hover:bg-mr-surface-subtle disabled:opacity-50"
                  >
                    <Meter signal={s.bars} tone={s.tone} />
                    <span className="min-w-0 flex-1 truncate text-mr-ui text-mr-ink">{o.label}</span>
                    <span className="shrink-0 text-mr-label text-mr-muted">
      ...
      <div className="primitive-card-footer flex items-center justify-between gap-3 border-t border-mr-line bg-mr-surface-soft">
        <span className="flex items-center gap-2">
          <Meter signal={signal.bars} tone={signal.tone} />
          <span className="text-mr-ui font-medium text-mr-ink-secondary">{confidenceLabel}</span>
        </span>
        ...
        <span className="-mr-0.5 flex items-center gap-2">
          {others.length > 0 && !locked && (
            <button
              ...
              className={`h-7 rounded-mr-control px-2.5 text-mr-ui font-medium shadow-mr-control
                transition-[background-color,transform] duration-100 active:scale-[0.96]
                ${open ? "bg-mr-surface-subtle text-mr-ink" : "bg-mr-surface text-mr-ink hover:bg-mr-surface-subtle"}`}
            >
              {labels.alternatives}
            </button>
          )}
