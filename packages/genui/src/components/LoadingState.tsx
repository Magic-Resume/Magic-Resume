    ...
    <div className="flex w-fit items-center gap-2.5">
      <span aria-hidden className="grid grid-cols-[repeat(3,4px)] gap-[1.5px]">
        {delays.map((d, i) => (
          <span
            key={i}
            className={`size-[4px] bg-mr-ink ${round ? "rounded-full" : "rounded-[1px]"}`}
            style={{
      ...
      <span
        ...
        className="shrink-0 bg-clip-text text-mr-caption font-medium whitespace-nowrap text-transparent"
        style={{
          backgroundImage:
      ...
      <span className="font-mono text-mr-overline text-mr-muted tabular-nums">
        {elapsed}
      </span>
    </div>
  );
