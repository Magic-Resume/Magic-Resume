...
function SourceChip() {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="ml-0 mr-1 inline-flex h-4.5 translate-y-[-1px] items-center gap-1 rounded-mr-compact
        bg-mr-sunk pr-[3px] pl-[3px] align-middle font-mono text-mr-micro-plus text-mr-ink-secondary shadow-mr-control
        transition-colors duration-150 hover:bg-mr-surface-soft hover:text-mr-ink"
...
export default function StreamingText() {
  ...
  return (
    <div className="min-h-[15.5rem] w-full max-w-95">
      <p className="text-mr-caption leading-relaxed text-mr-ink">
        {TOKENS.slice(0, count).map((token, i) =>
          token.cite ? (
            ...
            <span
              ...
              style={{ animation: "stream-in 420ms var(--narrate-ease) both" }}
            >
              {token.text}{" "}
            </span>
          ),
        ...
        {!done && (
          <span
            className="ml-0.5 inline-block h-3 w-0.5 translate-y-0.5 rounded-full bg-mr-ink"
      ...
      >
        {ACTION_ICONS.map((icon, i) => (
          <button
            ...
            type="button"
            aria-label="Action"
            className="flex size-6 items-center justify-center rounded-md text-mr-muted
              transition-colors duration-100 hover:bg-mr-surface-subtle hover:text-mr-ink-secondary"
          >
        ...
        <button
          ...
          className="ml-1.5 flex items-center gap-1.5 rounded-md px-1 py-0.5 text-left transition-colors duration-150 hover:bg-mr-surface-soft"
        >
          <span className="flex -space-x-1">
            {SOURCES.map((source) => (
              <img
                ...
                alt=""
                className="source-avatar size-3.5 rounded-full bg-mr-surface shadow-[0_0_0_1.5px_var(--mr-desk)]"
              />
            ))}
          </span>
          <span className="text-mr-overline text-mr-ink-secondary">10 sources</span>
        </button>
      </div>
      ...
      >
        <div className="overflow-hidden">
          <div className="mt-1.5 flex flex-col rounded-mr-card bg-mr-sunk p-1 shadow-mr-control">
            {SOURCES.map((source) => (
              <a
                key={source.domain}
                href={source.href}
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-2 rounded-md px-1.5 py-1 text-mr-overline text-mr-ink-secondary transition-colors duration-150 hover:bg-mr-surface-soft hover:text-mr-ink"
              >
                <img src={sourceImage(source)} alt="" className="source-avatar size-4 rounded" />
                <span className="animated-underline">{source.name}</span>
                <span className="ml-auto font-mono text-mr-micro-plus text-mr-muted">{source.domain}</span>
              </a>
            ))}
          </div>
        </div>
      ...
      <div
        ...
        style={{ opacity: done ? 1 : 0, pointerEvents: done ? "auto" : "none" }}
      >
        <p className="text-mr-overline font-medium text-mr-ink-secondary">Follow-ups</p>
        <div className="mt-0.5 flex flex-col">
          {FOLLOW_UPS.map((text, i) => (
            <button
              key={text}
              className="-mx-1.5 flex items-center gap-2 rounded-[7px] border-b border-mr-line
                px-1.5 py-1.5 text-left text-mr-ui text-mr-ink transition-colors
                duration-100 hover:bg-mr-surface-subtle"
              style={
                done
                  ? { animation: `fade-up 350ms var(--narrate-ease) ${i * 90}ms both` }
                  : { opacity: 0 }
              }
            >
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--mr-muted)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
