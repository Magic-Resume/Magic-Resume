 ...
 * 计算时单元格逐行落定。
 *
 * DOM、类名和交互遵循 GenUI 的 RecordsTable 视觉契约。
...
export function RecordsTag({ name, color }: { name: string; color?: string }) {
  return (
    <span className="records-tag" style={{ "--tag-base": color ?? "var(--mr-muted)" } as CSSProperties}>
      {name}
    </span>
  );
}
...
function MiniSwitch({ on, onToggle, label }: { on: boolean; onToggle: () => void; label: string }) {
  return (
    <button
      ...
      style={{ background: on ? "var(--mr-accent)" : "var(--mr-line-strong)" }}
    >
      <span
        className="absolute top-0.5 left-0.5 size-3.5 rounded-full bg-white shadow-mr-control transition-transform duration-150"
        style={{ transform: on ? "translateX(12px)" : "translateX(0)", transitionTimingFunction: "var(--narrate-ease)" }}
      />
    </button>
  );
}
...
function ConfigRow({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="relative flex h-8 items-center justify-between">
      <span className="text-mr-caption text-mr-muted">{label}</span>
      {children}
    </div>
  );
}
...
function ConfigPicker({
  ...
  return (
    <div
      role="menu"
      aria-label={label}
      className="absolute left-full top-0 z-30 ml-5 w-[210px] rounded-xl bg-mr-surface p-1.5 shadow-mr-panel"
      style={{ animation: "pop-in 140ms var(--narrate-ease) both", transformOrigin: "top left" }}
    >
      <div className="px-2 pb-1 pt-0.5 text-mr-label-tight font-medium text-mr-muted">{label}</div>
      <GlideMenu className="flex flex-col gap-px">
        {options.map((option) => (
          <button
            ...
            className="relative z-10 flex h-8 w-full items-center gap-1.5 rounded-mr-control px-1.5 text-left text-mr-caption font-medium text-mr-ink"
          >
            <span className="flex size-4 shrink-0 items-center justify-center text-mr-ink-secondary">{option.icon}</span>
            <span className="min-w-0 flex-1 truncate">{option.label}</span>
            <span className={selected === option.value ? "text-mr-ink" : "invisible"}>
...
function InputPicker({
  ...
  return (
    <div
      role="menu"
      aria-label={title}
      className="absolute left-full top-0 z-30 ml-5 w-[220px] rounded-xl bg-mr-surface p-1.5 shadow-mr-panel"
      style={{ animation: "pop-in 140ms var(--narrate-ease) both", transformOrigin: "top left" }}
    >
      <div className="px-2 pb-1 pt-0.5 text-mr-label-tight font-medium text-mr-muted">{title}</div>
      <GlideMenu className="flex flex-col gap-px">
        {options.map((option) => {
          ...
          return (
            <button
              ...
              className="relative z-10 flex h-8 w-full items-center gap-1.5 rounded-mr-control px-1.5 text-left text-mr-caption font-medium text-mr-ink"
            >
              <span className={`flex size-4 shrink-0 items-center justify-center rounded-mr-compact border ${checked ? "border-mr-accent bg-mr-accent text-white" : "border-mr-line-strong text-transparent"}`}>
  ...
  return (
    <div className={`records-shell${fill ? " is-fill" : ""} ${className}`}>
                ...
                <div className="flex h-[35px] items-center gap-1 px-2">
                  {canAddColumn && (
                    <button
                      ...
                      className="flex size-7 items-center justify-center rounded-[7px] text-mr-ink-secondary transition-colors duration-100 hover:bg-mr-surface-soft hover:text-mr-ink"
                    >
                  ...
                  <button
                    ...
                    className="flex size-7 items-center justify-center rounded-[7px] text-mr-muted transition-colors duration-100 hover:bg-mr-surface-soft hover:text-mr-ink"
                  >
      ...
      {prop && propColumn && meta && (
        <div
          data-recpop
          className="fixed z-50 w-[320px] rounded-mr-panel bg-mr-surface px-3 pt-3 pb-1.5 shadow-mr-panel"
          style={{ top: prop.y, left: prop.x, animation: "pop-in 160ms var(--narrate-ease) both", transformOrigin: "top left" }}
        >
          <div className="pb-2 text-mr-body-tight font-medium text-mr-ink">{propColumn.label}</div>
          ...
          {meta.type && (
            <ConfigRow label={labels.type}>
              <button
                ...
                className="flex items-center gap-1.5 rounded-md px-1.5 py-1 text-mr-caption font-medium text-mr-ink transition-colors duration-100 enabled:hover:bg-mr-surface-soft"
              >
                <span className="text-mr-ink-secondary"><Icon size={14}>{RECORDS_TYPE_GLYPHS[meta.type] ?? RECORDS_TYPE_GLYPHS.Text}</Icon></span>
                {labels.types[meta.type] ?? meta.type}
                {meta.editable && <span className="text-mr-muted"><Icon size={12} strokeWidth={2.2}><path d="M9 6l6 6-6 6" /></Icon></span>}
              </button>
          ...
          {meta.tool && (
            <ConfigRow label={labels.tool}>
              <button
                ...
                className="flex items-center gap-1.5 rounded-md px-1.5 py-1 text-mr-caption font-medium text-mr-ink transition-colors duration-100 enabled:hover:bg-mr-surface-soft"
              >
                <span className={meta.toolKind === "model" ? "text-mr-accent" : "text-mr-ink-secondary"}>
                  {meta.toolKind === "model"
                ...
                {meta.tool}
                {modelOptions && <span className="text-mr-muted"><Icon size={12} strokeWidth={2.2}><path d="M9 6l6 6-6 6" /></Icon></span>}
              </button>
            ...
            <ConfigRow label={labels.grounding}>
              <span className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="flex size-6 items-center justify-center rounded-md text-mr-muted transition-colors duration-100 hover:bg-mr-surface-soft hover:text-mr-ink"
                >
                  <Icon size={13}><g><circle cx="12" cy="12" r="9" /><path d="M12 8h.01M11 12h1v4h1" /></g></Icon>
                </button>
              </span>
              {groundingHelpOpen && (
                <div className="absolute right-0 top-[30px] z-30 w-[230px] rounded-mr-card px-3 py-2.5 text-mr-overline leading-relaxed shadow-mr-panel" style={{ color: "var(--tooltip-fg)", background: "var(--tooltip-bg)" }} role="status">
          ...
          {computed && (
            <ConfigRow label={labels.inputs}>
              <button
                ...
                className="flex max-w-[220px] items-center gap-1.5 rounded-md px-1.5 py-1 text-mr-caption text-mr-ink-secondary transition-colors duration-100 hover:bg-mr-surface-soft hover:text-mr-ink"
              >
                {selectedInputs.length ? (
                  <span className="flex min-w-0 items-center gap-1">
                    {selectedInputs.slice(0, 2).map((input) => (
                      <span key={input} className="max-w-[92px] truncate rounded-mr-compact bg-mr-accent-tint px-1.5 py-0.5 text-mr-overline font-medium text-mr-accent-ink">{input}</span>
                    ))}
                    {selectedInputs.length > 2 && <span className="text-mr-label font-medium text-mr-muted">+{selectedInputs.length - 2}</span>}
                  </span>
                ) : (
                  <span>{labels.selectInputs}</span>
                )}
                <span className="shrink-0 text-mr-muted"><Icon size={12} strokeWidth={2.2}><path d="M9 6l6 6-6 6" /></Icon></span>
              </button>
              {configMenu === "inputs" && (
                <InputPicker
          ...
          {computed && (
            <div
              ...
              aria-multiline="true"
              spellCheck
              className="mt-2 min-h-[88px] cursor-text rounded-mr-card bg-mr-sunk p-3 text-mr-caption leading-relaxed shadow-mr-control outline-none transition-[box-shadow] duration-150 focus:shadow-[0_0_0_2px_var(--mr-accent)]"
            >
              {meta.prompt ? (
                <span className="text-mr-ink">
                  {meta.prompt.before}
                  {meta.prompt.chip && <span contentEditable={false} className="rounded-mr-compact bg-mr-accent-tint px-1.5 py-0.5 text-mr-overline font-medium text-mr-accent-ink">{meta.prompt.chip}</span>}
                  {meta.prompt.after}
                </span>
              ) : (
                <span className="text-mr-muted">{labels.promptPlaceholder}</span>
              )}
            </div>
          )}
          ...
          {computed && onCalculate && (
            <button
              ...
              className="mt-2.5 flex h-9 w-full items-center justify-center gap-2 rounded-[9px] text-mr-ui font-medium text-mr-ink shadow-mr-control transition-[background-color,transform] duration-150 hover:bg-mr-surface-soft active:scale-[0.98] disabled:opacity-60"
            >
          ...
          <GlideMenu className="mt-3 flex flex-col gap-0.5 border-t border-mr-line pt-2" highlightClassName="-inset-x-1.5 rounded-mr-control bg-mr-surface-soft">
            <button
              data-menu-row
              type="button"
              ...
              className="relative z-10 -mx-1.5 flex h-8 items-center gap-2.5 rounded-mr-control px-1.5 text-left text-mr-caption leading-none text-mr-ink transition-transform duration-150 active:scale-[0.96]"
            >
              <span className={pinnedColumns.has(prop.key) ? "text-mr-accent" : "text-mr-ink-secondary"}><Icon size={15}><path d="M12 17v5M8 3h8l-1 7 3 3H6l3-3-1-7z" /></Icon></span>
            ...
            <button
              ...
              className="relative z-10 -mx-1.5 flex h-8 items-center gap-2.5 rounded-mr-control px-1.5 text-left text-mr-caption leading-none text-mr-ink transition-transform duration-150 active:scale-[0.96]"
            >
              <span className={moreSettingsOpen ? "text-mr-ink" : "text-mr-ink-secondary"}><Icon size={15}><g><circle cx="12" cy="12" r="3" /><path d="M12 2v3M12 19v3M4.9 4.9l2.1 2.1M17 17l2.1 2.1M2 12h3M19 12h3M4.9 19.1 7 17M17 7l2.1-2.1" /></g></Icon></span>
              <span className="flex-1">{labels.moreSettings}</span>
              <span className={`text-mr-muted transition-transform duration-150 ${moreSettingsOpen ? "rotate-90" : ""}`}><Icon size={12} strokeWidth={2.2}><path d="M9 6l6 6-6 6" /></Icon></span>
            </button>
            {onHideColumn && (
              <button
                data-menu-row
                ...
                className="relative z-10 -mx-1.5 flex h-8 items-center gap-2.5 rounded-mr-control px-1.5 text-left text-mr-caption leading-none text-mr-ink transition-transform duration-150 active:scale-[0.96]"
              >
                <span className="text-mr-ink-secondary"><Icon size={15}><g><path d="M10.6 5.1A9.8 9.8 0 0 1 12 5c7 0 10 7 10 7a16.3 16.3 0 0 1-2.1 3M6.6 6.6A16 16 0 0 0 2 12s3 7 10 7a9.7 9.7 0 0 0 5.4-1.6M3 3l18 18" /><path d="M9.9 9.9a3 3 0 0 0 4.2 4.2" /></g></Icon></span>
                {labels.hideFromView}
              </button>
            )}
          </GlideMenu>
          ...
          {moreSettingsOpen && (
            <div className="mt-2 border-t border-mr-line pt-2" style={{ animation: "fade-up 160ms var(--narrate-ease) both" }}>
              <div className="pb-1 text-mr-label-tight font-medium text-mr-muted">{labels.behavior}</div>
      ...
      {addOpen && canAddColumn && (
        <div
          data-recpop
          className="fixed z-50 w-[260px] rounded-mr-panel bg-mr-surface p-1.5 shadow-mr-panel"
          style={{ top: addOpen.y, left: addOpen.x, animation: "pop-in 160ms var(--narrate-ease) both", transformOrigin: "top left" }}
        >
          <div className="px-2 pb-1 pt-1 text-mr-overline font-medium text-mr-muted">{labels.newProperty}</div>
          <GlideMenu className="flex flex-col gap-px">
            {addTypes.map((type) => (
              <button
                key={type}
                ...
                className="relative z-10 flex h-9 w-full items-center gap-2.5 rounded-mr-control px-2 text-left text-mr-caption text-mr-ink"
              >
                <span className="text-mr-ink-secondary"><Icon size={15}>{RECORDS_TYPE_GLYPHS[type]}</Icon></span>
      ...
      {tableMenuOpen && (
        <div
          data-recpop
          className="fixed z-50 w-[220px] rounded-mr-panel bg-mr-surface p-1.5 shadow-mr-panel"
          style={{ top: tableMenuOpen.y, left: tableMenuOpen.x, animation: "pop-in 160ms var(--narrate-ease) both", transformOrigin: "top right" }}
        >
          <div className="px-2 pb-1 pt-1 text-mr-overline font-medium text-mr-muted">{labels.tableOptions}</div>
          <GlideMenu className="flex flex-col gap-px">
            {canAddColumn && (
              <button
                data-menu-row
                ...
                className="relative z-10 flex h-9 w-full items-center gap-2.5 rounded-mr-control px-2 text-left text-mr-caption text-mr-ink"
              >
                <span className="text-mr-ink-secondary"><Icon size={15} strokeWidth={2}><path d="M12 5v14M5 12h14" /></Icon></span>
                {labels.addProperty}
              </button>
            )}
            <button
              ...
              className="relative z-10 flex h-9 w-full items-center gap-2.5 rounded-mr-control px-2 text-left text-mr-caption text-mr-ink"
            >
              <span className="text-mr-ink-secondary"><Icon size={15}><path d="M4 8h16M7 4 3 8l4 4M17 4l4 4-4 4M4 16h16" /></Icon></span>
            ...
            <button
              ...
              className="relative z-10 flex h-9 w-full items-center gap-2.5 rounded-mr-control px-2 text-left text-mr-caption text-mr-ink"
            >
              <span className="text-mr-ink-secondary"><Icon size={15}><path d="M3 12a9 9 0 1 0 3-6.7M3 4v6h6" /></Icon></span>
            ...
            {selectable && (
              <>
                ...
                <button
                  ...
                  className="relative z-10 flex h-9 w-full items-center gap-2.5 rounded-mr-control px-2 text-left text-mr-caption text-mr-ink"
                >
                  <span className="text-mr-ink-secondary"><Icon size={15}><path d="M5 5l14 14M19 5 5 19" /></Icon></span>
