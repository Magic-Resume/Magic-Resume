  ...
  return (
    <div className="flex w-full max-w-80 flex-col items-stretch">
      <div className="w-full self-start overflow-hidden rounded-mr-card bg-mr-surface shadow-mr-panel">
        <div key={qi} className="primitive-card-pad" style={{ animation: "fade-up 350ms var(--narrate-ease) both" }}>
          <span className="text-mr-caption font-medium text-mr-ink">{question.q}</span>

          {question.answered !== undefined ? (
            <div className="mt-2 flex items-center gap-1.5 text-mr-ui text-mr-ink-secondary">
              <span
                className="flex size-4 shrink-0 items-center justify-center rounded-full bg-mr-success text-[#fff]"
                style={{ animation: "pop-in 300ms var(--narrate-ease) both" }}
              >
            ...
            <GlideMenu
              ...
              highlightClassName="-inset-x-1.5 rounded-mr-control bg-mr-surface-soft"
            >
              {question.options.map((option, i) => {
                ...
                return (
                  <button
                    ...
                    disabled={locked}
                    onClick={() => toggle(i)}
                    className="relative z-10 -mx-1.5 flex items-center gap-2 rounded-mr-control px-1.5 py-1 text-left transition-colors duration-100 disabled:opacity-50"
                  >
                    <span
                      className={`flex size-4 shrink-0 items-center justify-center transition-colors duration-200
                        ${question.type === "radio" ? "rounded-full" : "rounded-mr-compact"}
                        ${on ? "bg-mr-ink text-mr-desk" : "shadow-[inset_0_0_0_1.5px_var(--mr-line-strong)] text-transparent"}`}
                    >
                      {question.type === "radio" ? (
                        <span className="size-1.5 rounded-full bg-mr-desk transition-transform duration-200" style={{ transform: on ? "scale(1)" : "scale(0)" }} />
                      ) : (
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>
                      )}
                    </span>
                    <span className={`text-mr-caption transition-colors duration-200 ${on ? "text-mr-ink" : "text-mr-ink-secondary"}`}>
                      {option}
                    </span>
                  </button>
                );
              ...
              {freeTextPlaceholder ? (
                <label
                  data-menu-row
                  className="relative z-10 -mx-1.5 flex items-center gap-2 rounded-mr-control px-1.5 py-1 transition-colors duration-100"
                >
        ...
        <div className="primitive-card-footer flex items-center justify-between">
            ...
            {paged && (
              <>
                <button
                  ...
                  className="flex size-6 items-center justify-center rounded-mr-compact text-mr-muted transition-colors duration-100 enabled:hover:bg-mr-surface-soft enabled:hover:text-mr-ink-secondary disabled:opacity-35"
                >
                ...
                <span className="flex items-center gap-1">
                  {QUESTIONS.map((q, i) => (
                    <button
                      ...
                      style={
                        i === qi
                          ? { width: 9, height: 9, border: "2.5px solid var(--mr-ink)" }
                          : q.answered !== undefined
                            ? { width: 7, height: 7, background: "var(--mr-muted)" }
                            : { width: 7, height: 7, border: "1.5px solid var(--mr-muted)" }
                      }
                    />
                  ))}
                </span>
          ...
          {!locked && (
            <button
              ...
              disabled={!hasAnswer}
              onClick={() => commit(qi)}
              className="-mr-0.5 flex size-7 items-center justify-center rounded-mr-control transition-[background-color,color,transform] duration-200 enabled:active:scale-[0.96]"
              style={{
                background: hasAnswer ? "var(--mr-ink)" : "var(--mr-sunk)",
                color: hasAnswer ? "var(--mr-surface)" : "var(--mr-muted)",
                boxShadow: hasAnswer ? "inset 0 1px 0 rgba(255,255,255,0.14)" : "var(--shadow-mr-control)",
              }}
            >
