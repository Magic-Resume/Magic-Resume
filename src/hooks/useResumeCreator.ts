...
export const useResumeCreator = () => {
  ...
  const generateResume = useCallback(async () => {
    ...
    try {
        ...
        toast.success(t('common.notifications.draftGenerated'));
    } catch (error) {
        console.error('Error generating resume:', error);
        addMessage({ id: nanoid(), role: 'ai', content: t('common.notifications.generationError') });
    } finally {
  ...
  const sendMessage = useCallback(async (userMessage: string) => {
    ...
    try {
      ...
      addMessage({ id: aiMessageId, role: 'ai', content: t('common.notifications.generatingResume') });
      setIsLoading(false);
                    ...
                    if (resumeJsonStart !== -1) {
                      ...
                      if (resumeJsonEnd !== -1) {
                        ...
                        if (resumeData && typeof resumeData === 'object') {
                          ...
                          toast.success(t('common.notifications.dataApplied'));
                        } else {
